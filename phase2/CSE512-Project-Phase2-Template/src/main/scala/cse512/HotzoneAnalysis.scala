...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // Calculate number of points in each rectangle
    // Group the data frame by "rectangle" column, count the number of rows in each group and order by "rectangle" column
    val resultDf = joinDf.groupBy("rectangle").count().orderBy(asc("rectangle")).toDF()
    resultDf.show()
    return resultDf
  }
}
